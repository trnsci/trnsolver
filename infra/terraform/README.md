# Terraform — trnsolver Neuron CI Instance
...
user-data to finish (installs Neuron SDK, clones trnsolver), then stop it:
...
| `instance_tag` | `trnsolver-ci-trn1` | Must match `scripts/run_neuron_tests.sh [type]` arg |
| `vpc_id` | (required) | |
